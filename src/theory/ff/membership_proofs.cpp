...
void GBProof::reductionStart(CoCoA::ConstRefRingElem p)
{
  ...
  Trace("ff::trace") << "GBreduction start: " << p << std::endl;
...
{
  Assert(!d_reductionSeq.empty());
  Trace("ff::trace") << "GBreduction step: " << q << std::endl;
...
void GBProof::reductionEnd(CoCoA::ConstRefRingElem r)
{
  ...
  if (d_polyToMembership.count(rr) == 0 && rr != d_reductionSeq.front())
  {
    ...
    d_proof->addStep(conclusion, ProofRule::FF_R_DOWN, premises, reductorsSeq, true);
  }
  else
  {
